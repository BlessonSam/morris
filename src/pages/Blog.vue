<template>
    <q-page dark class="py-10">
       <div dark class="container text-center pb-10 pt-20 space-y-3">
           ...
           <div class="text-xs text-sectionSubTitle">Articles by our team and students</div>
       </div>
       <div class="container pt-10 grid grid-cols-1 sm:grid-cols-2  md:grid-cols-3 gap-12">
             <div v-for="i in 10" :key="i" @click="goToArticle(article.id)" class="bg-primary rounded-xl overflow-hidden hover:shadow-2xl cursor-pointer">
                <!-- <div class="w-full overflow-hidden  ">
                    <q-img :ratio="4/3" class="object-cover w-full h-52" :src="article.image" />

                </div> -->
                <div class="p-5 lg:p-7  space-y-7 h-1/2">
                   <div class="font-semibold text-bigText text-2xl">{{article.title}}</div>
                   <div class="text-sm text-sectionTitle">{{article.subtitle}}</div>
                   <div class="font-thin text-xs text-sectionSubTitle">{{article.createdAt}}</div>
                </div>
             </div>
       </div>
    </q-page>
